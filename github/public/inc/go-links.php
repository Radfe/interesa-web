...
function interessa_is_placeholder_go_url(string $url): bool {
  return str_contains($url, 'REPLACE_') || str_contains($url, 'example.com');
}
...
function interessa_go_links(): array {
  ...
  foreach (['/affiliate_simple_edit.csv', '/affiliate_links.csv'] as $csvRel) {
    ...
    $header = fgetcsv($fh, 0, ',', '"', '');
    $sep = ',';
    if ($header && count($header) === 1 && strpos((string) $header[0], ';') !== false) {
      fclose($fh);
      $fh = fopen($csv, 'r');
      if (!$fh) continue;
      $header = fgetcsv($fh, 0, ';', '"', '');
    ...
    foreach ((array) $header as $i => $h) {
      $hmap[$i] = strtolower(trim((string) $h, " \t\n\r\0\x0B\"'"));
    }

    $iC = array_search('code', $hmap, true);
    $iU = array_search('url', $hmap, true);
    if ($iU === false) {
      $iU = array_search('deeplink', $hmap, true);
    }
    ...
    if ($iC !== false && $iU !== false) {
      while (($row = fgetcsv($fh, 0, $sep, '"', '')) !== false) {
        ...
        if ($c !== '' && $u !== '' && stripos($u, 'http') === 0 && !interessa_is_placeholder_go_url($u)) {
          $DEFAULT[$c] = $u;
        }
      }
    }
